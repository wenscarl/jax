...
def update_global_scale(config, new_global_scale):
  """
  Sets the global_scale for a list of BlockScaleConfig objects.

  Args:
      configs (list): A list of BlockScaleConfig objects.
      new_global_scale: The new global_scale value to set.

  Returns:
      list: The updated list of BlockScaleConfig objects.
  """
...
class ScaledMatmulTest(jtu.JaxTestCase):
  ...
  @jtu.sample_product(
      ...
      block_scale_configs=[nvfp4_configs,],
  )
  @jtu.run_on_devices("cuda")
...
class MxFp8ScaledDotGeneralTest(jtu.JaxTestCase):
  ...
  def test_dot_general_nvfp4(self, configs, output_type):
    ...
    a = cast_to_representable(
        ...
        block_scale_configs_nvfp4[0].data_type,
    )
    b = cast_to_representable(
        b_raw,
        #jax.random.uniform(k2, b_shape, minval=-1.0, dtype=output_type),
        block_scale_configs_nvfp4[1].data_type,
    )
    a = a_raw
    b = b_raw
    ...
    block_scale_configs_nvfp4[0] = update_global_scale(block_scale_configs_nvfp4[0],
                                                       amax_a / (data_max * scale_max)) 
    block_scale_configs_nvfp4[1] = update_global_scale(block_scale_configs_nvfp4[1],
                                                       amax_b / (data_max * scale_max)) 

    print(a, b)

    scaled_dot_general = partial(
        scaled_dot_general_wrapper,
        configs=block_scale_configs_nvfp4
    )
    def fwd(a, b, is_ref=False):
      ...
      if is_ref:
        ...
        *bs_b, k_b = b_nvfp4.shape
        ...
        a = a_nvfp4.astype(jnp.float32).reshape(*bs_a, k_a // 16, 16) * jnp.expand_dims(a_scale.astype(jnp.float32), -1)
        b = b_nvfp4.astype(jnp.float32).reshape(*bs_b, k_b // 16, 16) * jnp.expand_dims(b_scale.astype(jnp.float32), -1)
        a = a.reshape(*bs_a, k_a) * block_scale_configs_nvfp4[0].global_scale
        b = b.reshape(*bs_b, k_b) * block_scale_configs_nvfp4[1].global_scale
